...
const newBlock = `                    {/* A. Paytm Gateway Block */}
                    <div className="bg-[#161622] p-3.5 rounded-xl border border-white/5 space-y-3">
                      <div className="flex justify-between items-center">
                        ...
                        <input 
                          type="checkbox"
                          checked={appSettings.paytmEnabled && !!appSettings.paytmMerchantKey}
                          onChange={e => setAppSettings({...appSettings, paytmEnabled: e.target.checked})}
                          disabled={!appSettings.paytmMerchantKey}
                          className="w-4 h-4 text-gold-500 bg-neutral-950 border-white/10 rounded cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                        />
                      </div>
