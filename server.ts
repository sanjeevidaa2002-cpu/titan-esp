...
import { initializeFirestore, doc, getDoc, setDoc, updateDoc, collection, addDoc, getDocs, runTransaction, query, where } from "firebase/firestore";
  ...
  app.post("/api/admin/payments/complete", async (req, res) => {
    try {
      ...
      await runTransaction(db, async (t) => {
        ...
        // Update user wallet
        t.update(userRef, {
          depositBalance: newDepositWallet
        });
        ...
        t.set(notifyRef, {
          ...
          message: `Your payment of ₹${txnData.amount} has been verified and your wallet has been credited successfully.`,
          type: "success",
          dateTime: new Date().toISOString(),
          isRead: false
        });
